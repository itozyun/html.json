...
## 4. `<pre>` `<listing>` 内の改行文字前のスペース、タブ

`<pre>` `<listing>` 下のテキストノードには、特別なルールで最小化を行う．
...
2. `</pre>` の直前の行の後ろの半角スペース・タブも削除する．(`trimWhitespaces:"aggressive"`)
